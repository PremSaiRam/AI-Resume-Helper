...
export default function Login({ onLogin }) {
  ...
  const handleGoogleLogin = () => {
    ...
    if (storedName) {
      onLogin(storedName); // proceed to dashboard
    } else {
  ...
  const handleNameSubmit = () => {
    if (!displayName.trim()) return alert("Enter a display name");
  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <p style={styles.lead}>
          Sign in with Google to upload resumes, view historic scores and get
        ...
        {!showNameInput ? (
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            ...
            />
            <button onClick={handleNameSubmit} style={styles.googleBtn}>
              Continue
            </button>
          </div>
        )}
        ...
        <div style={{ marginTop: 18, color: "#6b7280" }}>
          <small>
            We only store your resume analyses locally (no DB). Sessions are
...
const styles = {
  page: {
    ...
    placeItems: "center",
    background:
      "radial-gradient(circle at 10% 20%, #0f172a 0%, rgba(12, 9, 39, 0.7) 20%, rgba(18, 15, 50, 0.6) 40%, rgba(14, 73, 93, 0.6) 100%)",
    padding: 20,
  },
  card: {
    width: 760,
    ...
    padding: "36px 36px",
    boxShadow:
      "0 12px 40px rgba(2,6,23,0.6), inset 0 1px 0 rgba(255,255,255,0.03)",
    color: "#fff",
